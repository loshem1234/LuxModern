...
4. Go to your new service's **Variables** tab and add:
   ...
   have one yet.
5. Railway will redeploy automatically after you save the variable. Once
   it's live, open the generated `*.up.railway.app` URL — your site should
   load, and the home page chat should respond for real.
6. Optional: under **Settings → Networking**, add a custom domain once
...
holds `ANTHROPIC_API_KEY` as a server-side environment variable and adds it
to the request it makes to Anthropic — that key never appears in any file
the browser downloads, so viewing page source or dev tools won't expose it.
This is the proper setup for a public, live site (unlike a GitHub-Pages-only
